...
def accessory_gen(loop_counter):
    all_accessories =['cigarette', 'upsidedown']
    accessories_T_F = random.choice([True, False])
    
    if accessories_T_F:
        num_of_accessories = loop_counter
        accessories = random.choices(all_accessories, weights=None, cum_weights=None, k=num_of_accessories)
    else:
        accessories = 'None'
        num_of_accessories = 'None'
...
def img_layering(i, img_data, accessories, dimensions, PATH):
    if accessories[i] == 'None':
        img_data.save(f'{PATH}/duck-{i+1}.png')
    
    else:
        if accessories[i] == 'cigarette':
            cigarette = Image.open('accessories/cigarette.png')
            cigarette = cigarette.resize(dimensions, resample=0)
            img_data.paste(cigarette, (0,0), cigarette)
            img_data.save(f'{PATH}/duck-{i+1}.png')
        
        elif accessories[i] == 'upsidedown':
            img_data = ImageOps.flip(img_data)
            img_data.save(f'{PATH}/duck-{i+1}.png')
...
def main_loop():
    ...
    for i in range(0, loop_counter):
        ...
        dimensions, img_data = img_generator(data, PF, EW, EC, BC, OT, BG, BK)

        accessories, num_of_accessories = accessory_gen(loop_counter)
        
        img_layering(i, img_data, accessories, dimensions, PATH)
